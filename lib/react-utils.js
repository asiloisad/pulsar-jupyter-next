...
const REQUIRED_API_VERSION = "1.1.0";
...
function getHydrogenPackagePath() {
  if (!hydrogenPackagePath) {
    hydrogenPackagePath = atom.packages.getLoadedPackage("hydrogen-next").path;
  }
  return hydrogenPackagePath;
}
 ...
 * @returns {Object} The shared module
 */
...
function unmount(container) {
  ...
  if (root) {
    // Always remove from map first to prevent double-unmount attempts
    rootMap.delete(container);
    try {
      root.unmount();
      return true;
    } catch (e) {
      // Ignore cleanup errors - DOM may have been modified externally
    }
  }
