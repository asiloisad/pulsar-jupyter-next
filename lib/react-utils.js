...
function getHydrogenShared() {
  ...
  if (!pkgPath) {
    console.error('[jupyter-next] hydrogen-next package not found - this package requires hydrogen-next')
    return null
  }
  ...
  try {
    ...
    console.error('[jupyter-next] Failed to load hydrogen-next shared module:', e.message)
    return null
  }
}
