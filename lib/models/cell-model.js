...
class CellModel {
  constructor({ id, type, source, outputs, executionCount, metadata, executionTime }) {
    this.id = id
    this.type = type || 'code'
    this.source = source || ''
  ...
  addOutput(output) {
    const shared = getHydrogenShared()
    if (!shared) {
      console.error('[jupyter-next] hydrogen-next shared module required for addOutput')
      return
    }
    ...
    if (output.output_type === 'stream') {
      ...
      if (lastOutput && lastOutput.output_type === 'stream' && lastOutput.name === output.name) {
        // Merge with last output using shared utility
        lastOutput.text = shared.escapeCarriageReturnSafe((lastOutput.text || '') + (output.text || ''))
        this.emitter.emit('did-change')
        return
      }
    }
  ...
  toJSON() {
    ...
    if (this.type === 'code') {
      ...
      cell.outputs = this.outputs.map(output =>
        shared?.outputToNotebookFormat ? shared.outputToNotebookFormat(output) : output
      )
    }
