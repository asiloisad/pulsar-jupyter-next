...
class NotebookDocument {
  ...
  async load() {
    ...
    try {
      ...
      if (this.cells.length === 0) {
        this.cells.push(new CellModelClass({
          id: uuidv4(),
          type: 'code',
          source: '',
          outputs: [],
  ...
  async connectToKernel(kernelSpec) {
    try {
      ...
      this.kernel = await this.kernelManager.getOrStartKernel(kernelSpec.name || kernelSpec)
  ...
  async requestKernelConnection() {
    ...
    const language = this.metadata?.kernelspec?.language || this.metadata?.language_info?.name || null

    const picker = new KernelPicker(this.kernelManager, { preferredKernelName, language })
    return picker.show()
  }
  ...
  async executeCell(index, callbacks = {}) {
    ...
    if (atom.config.get('jupyter-next.execution.clearOutputBeforeRun')) {
      cell.clearOutputs()
    }
  ...
  moveCells(indices, targetIndex) {
    ...
    this.setModified(true)
    this.emitter.emit('did-change')
    this.emitter.emit('did-move-cells', { indices: sortedIndices, targetIndex: adjustedTarget })
  }

  updateCellSource(index, source) {
    if (index >= 0 && index < this.cells.length) {
      const cell = this.cells[index]
      // Only mark as modified if the source actually changed
