...
class NotebookDocument {
  ...
  async requestKernelConnection() {
    const kernelSpec = await this.getPreferredKernelSpec();
    if (kernelSpec) {
      return kernelSpec;
    }
    ...
    const preferredKernelDisplayName = this.metadata?.kernelspec?.display_name || null;
    const language =
      this.metadata?.kernelspec?.language || this.metadata?.language_info?.name || null;
  ...
  async getPreferredKernelSpec() {
    const preferredKernelName = this.metadata?.kernelspec?.name;
    const preferredKernelDisplayName = this.metadata?.kernelspec?.display_name;
    if (!preferredKernelName && !preferredKernelDisplayName) return null;

    try {
      const specs = await this.kernelManager.getKernelSpecs();
      return specs.find((spec) => this._matchesKernelMetadata(spec)) || null;
    } catch (error) {
      return null;
    }
  }

  _matchesKernelMetadata(spec) {
    const kernelspec = this.metadata?.kernelspec || {};
    return (
      this._matchesKernelValue(kernelspec.name, spec.name) ||
      this._matchesKernelValue(kernelspec.name, spec.displayName) ||
      this._matchesKernelValue(kernelspec.name, spec.display_name) ||
      this._matchesKernelValue(kernelspec.display_name, spec.name) ||
      this._matchesKernelValue(kernelspec.display_name, spec.displayName) ||
      this._matchesKernelValue(kernelspec.display_name, spec.display_name)
    );
  }
  ...
  async executeCell(index, callbacks = {}) {
    ...
    if (atom.config.get("jupyter-next.output.clearBeforeRun")) {
      cell.clearOutputs();
    }
