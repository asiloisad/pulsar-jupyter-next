...
class CellView {
  constructor(props) {
    ...
    this._maxInputHeightDisposable = atom.config.observe("jupyter-next.maxInputHeight", () => {
      this.applyMaxInputHeight();
    });
    this._maxOutputHeightDisposable = atom.config.observe("jupyter-next.maxOutputHeight", () => {
      this.applyOutputHeight();
    });
    this._scrollOutputDisposable = atom.config.observe("jupyter-next.scrollOutput", () => {
      this.applyOutputHeight();
    });

    if (props.cell && props.cell.running) {
      this.startLiveTimer();
    }
  }

  startLiveTimer() {
    if (this._liveTimerInterval) return;
    this._liveTimerInterval = setInterval(() => this.refreshTimeIndicator(), 50);
  }
  ...
  refreshTimeIndicator() {
    ...
    if (this._lastState.timeStr === timeStr) return;

    if (timeStr) {
      if (!this._cachedElements.timeIndicator && this._cachedElements.gutter) {
  ...
  render() {
    ...
    runBtn.onclick = (e) => {
      e.stopPropagation();
      if (editor) editor.runCellAt(this.props.index);
    };
    actions.appendChild(runBtn);

    if (cell.type === "code") {
      const clearBtn = document.createElement("button");
      clearBtn.className = "btn btn-xs icon icon-circle-slash";
  ...
  update(props) {
    ...
    const wasRunning = oldProps.cell && oldProps.cell.running;
    const isRunning = props.cell && props.cell.running;
    if (isRunning && !this._liveTimerInterval) {
      this.startLiveTimer();
  ...
  applyOutputHeight() {
    ...
    if (!container) return;
    const max = atom.config.get("jupyter-next.maxOutputHeight");
    const scroll = atom.config.get("jupyter-next.scrollOutput");
    if (scroll && max > 0) {
  ...
  applyMaxInputHeight() {
    if (!this.editorContainer) return;
    const max = atom.config.get("jupyter-next.maxInputHeight");
    if (max > 0) {
  ...
  destroy() {
    ...
    if (this._scrollOutputDisposable) {
      this._scrollOutputDisposable.dispose();
      this._scrollOutputDisposable = null;
    }
