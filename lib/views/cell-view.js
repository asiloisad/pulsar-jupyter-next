...
class CellView {
  ...
  render() {
    ...
    const showCellNumbers = atom.config.get('jupyter-next.notebook.showCellNumbers')
    ...
    this._cachedElements = { execCount: null, timeIndicator: null, gutter: null }
  ...
  getExecutionCountText() {
    ...
    if (cell.executionCount) {
      ...
      const hasError = cell.outputs && cell.outputs.some(o => o.output_type === 'error')
      if (hasError) {
  ...
  _doRenderOutputs() {
    ...
    const displayableOutputs = (cell.outputs || []).filter(output => {
      ...
      return type === 'stream' || type === 'execute_result' ||
             type === 'display_data' || type === 'error'
    })
    ...
    if (cell.type === 'code' && displayableOutputs.length > 0 && cell.outputVisible !== false) {
      ...
      if (this.outputView) {
        ...
        this.outputView.update({
          ...
          maxHeight: atom.config.get('jupyter-next.notebook.maxOutputHeight')
        })
      } else {
  ...
  setupEditor() {
    ...
    this.editor = atom.workspace.buildTextEditor({
      mini: false,
      lineNumberGutterVisible: atom.config.get('jupyter-next.display.lineNumbers'),
      softWrapped: atom.config.get('jupyter-next.display.wordWrap'),
      autoHeight: true
    })
    ...
    // Set font size if configured
    const fontSize = atom.config.get('jupyter-next.display.fontSize')
    if (fontSize > 0) {
  ...
  applyGrammar() {
    ...
    if (cell.type === 'markdown') {
      ...
      const language = document?.kernel?.language ||
      ...
      targetScopes = grammarScopes[language.toLowerCase()] || [`source.${language.toLowerCase()}`]
    }
  ...
  update(props) {
    ...
    const needsRebuild = typeChanged ||
    ...
    if (needsRebuild) {
      ...
      // But DON'T update if the editor has unsaved changes (dirty) to avoid race conditions
      if (this.editor && props.cell && !this._editorIsDirty) {
        const editorText = this.editor.getText()
        // Only update if the model source differs from both editor AND last known source
        // This means the change came from outside (e.g., file reload), not from editor
        if (props.cell.source !== editorText && props.cell.source !== this._lastKnownSource) {
