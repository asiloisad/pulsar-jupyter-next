...
class NotebookView {
  constructor(props) {
    ...
    document.addEventListener('mouseup', this._handleGlobalMouseUp = () => { this._mouseButtonDown = false })
  }
  ...
  buildToolbar() {
    ...
    cellTypeSelect.innerHTML = '<option value="code">Code</option><option value="markdown">Markdown</option><option value="raw">Raw</option>'
  ...
  renderCells() {
    ...
    const expectedOrder = cellsArray.map(cell => this.cellViews.get(cell.id)?.element).filter(Boolean)

    const needsReorder = currentChildren.length !== expectedOrder.length ||
  ...
  handleClick(event) {
    ...
    if (isCell && !isEditor) {
      ...
      // Clicked on background (not on any cell) - clear selection
      this.clearSelection()
    }
  }
  ...
  scrollToCell(index) {
    ...
    if (cells && cells[index]) {
      ...
      if (cellView && cellView.element) {
        cellView.element.scrollIntoView({ behavior: 'smooth', block: 'nearest' })
      }
    }
  }
