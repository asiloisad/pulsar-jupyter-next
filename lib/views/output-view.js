...
class OutputView {
  ...
  _getOutputsHash(outputs) {
    ...
    return outputs.map(o => {
      ...
      if (type === 'stream') {
        ...
        content = Object.keys(o.data).map(k => {
          const val = o.data[k]
  ...
  renderContent() {
    ...
    const style = scrollOutput && maxHeight
    ...
    if (shared && shared.Display) {
      ...
      console.error('[jupyter-next] hydrogen-next shared module is required for output rendering')
      this.renderFallback()
    }
  }
  ...
  renderFallback() {
    ...
    const sanitizeHtml = shared?.sanitizeHtml || this._sanitizeHtmlSimple.bind(this)
    ...
    outputs.forEach((output) => {
      ...
      div.className = `jupyter-output output-${output.output_type || 'unknown'}`
      ...
      switch (output.output_type) {
        ...
        case 'execute_result':
        case 'display_data':
          this.renderDisplayDataFallback(div, output.data, sanitizeHtml, ansiToHtml)
          break
        ...
        case 'error':
          ...
          if (output.traceback) {
            ...
            tb.innerHTML = output.traceback.map(line =>
  ...
  renderDisplayDataFallback(container, data, sanitizeHtml, ansiToHtml) {
    ...
    const mimeTypes = [
      ...
      'application/json', 'text/latex', 'text/markdown', 'text/plain'
    ]
