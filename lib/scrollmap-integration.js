...
class NotebookScrollmap {
  constructor(editor, Simplemap) {
    ...
    this.disposables.add(
      ...
      atom.config.onDidChange("jupyter-next.scrollmap.enabled", () => this.scheduleUpdate()),
      atom.config.onDidChange("jupyter-next.scrollmap.threshold", () => this.scheduleUpdate()),
      atom.config.onDidChange("jupyter-next.scrollmap.depth", () => this.scheduleUpdate()),
      atom.config.onDidChange("jupyter-next.input.maxHeight", () => this.scheduleUpdate()),
      atom.config.onDidChange("jupyter-next.output.maxHeight", () => this.scheduleUpdate()),
  ...
  update() {
    ...
    const items = [...selectionItems, ...headerItems, ...linterItems].sort(
      (a, b) => a.prc - b.prc,
    );
  ...
  isEnabled() {
    return atom.config.get("jupyter-next.scrollmap.enabled") !== false;
  }

  getThreshold() {
    return atom.config.get("jupyter-next.scrollmap.threshold") || 0;
  }

  getDepth() {
    return atom.config.get("jupyter-next.scrollmap.depth") || 0;
  }
  ...
  markerForLinterMessage(message, container) {
    ...
    return {
      ...
      click: () => this.editor?.revealLinterMessage?.(message),
    };
  }
