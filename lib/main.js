...
module.exports = {
    ...
    this.disposables.add(
      atom.commands.add(".tree-view", {
        "jupyter-next:open-source": (event) => {
          event.stopPropagation();
          this.openSelectedTreeViewSource();
        },
      }),
    );
  ...
  async openSource(filePath = null) {
    ...
    const buffer = await atom.project.bufferForPath(sourcePath);
    const editor = atom.workspace.buildTextEditor({ buffer });
    return atom.workspace.open(editor);
  },

  async openSelectedTreeViewSource() {
    const selectedPath = this.getSelectedTreeViewNotebookPath();
    if (!selectedPath) return;
    return this.openSource(selectedPath);
  },

  getSelectedTreeViewNotebookPath() {
    const treeViewPackage = atom.packages.getActivePackage("tree-view");
    const treeView = treeViewPackage?.mainModule?.getTreeViewInstance?.();
