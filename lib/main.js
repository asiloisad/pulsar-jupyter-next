...
module.exports = {
  ...
  activate() {
    ...
    if (this.useWorkspaceOpener()) {
      this.disableHydrogenNotebookOpener();
    }
    ...
    this.disposables.add(
      atom.config.onDidChange("jupyter-next.useOpener", ({ newValue }) => {
        if (newValue !== false) {
          this.disableHydrogenNotebookOpener();
  ...
  useWorkspaceOpener() {
    return atom.config.get("jupyter-next.useOpener") !== false;
  },

  disableHydrogenNotebookOpener() {
  ...
  async openSelectedTreeViewNotebook(event = null) {
    const selectedPath = this.getSelectedTreeViewNotebookPath(event);
    if (!selectedPath || !this.requireHydrogen()) return;
