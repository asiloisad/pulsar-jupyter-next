...
module.exports = {
  ...
  activate(state) {
    ...
    this.disposables.add(
      atom.commands.add("atom-workspace", {
        ...
        "jupyter-next:change-cell-to-markdown": () =>
        ...
        "jupyter-next:split-left-and-copy": () =>
          this.splitPane("horizontal", "before", true),
        "jupyter-next:split-right-and-copy": () =>
          this.splitPane("horizontal", "after", true),
        "jupyter-next:split-up-and-copy": () =>
          this.splitPane("vertical", "before", true),
        "jupyter-next:split-down-and-copy": () =>
          this.splitPane("vertical", "after", true),
      })
    );
    ...
    try {
      ...
      if (this.documentRegistry) {
        try {
          ...
          console.error(
            "[jupyter-next] Error destroying document registry:",
            e
          );
        }
        this.documentRegistry = null;
      ...
      if (this.kernelManager) {
        try {
          ...
          console.error(
            "[jupyter-next] Error shutting down kernel manager:",
            e
          );
        }
        this.kernelManager = null;
      ...
      for (const paneContainer of [
        ...
        for (const pane of paneContainer.getPanes()) {
          for (const item of pane.getItems()) {
            ...
            if (
              existingPath &&
              path.normalize(existingPath).toLowerCase() === normalizedUri
            ) {
  ...
  async showKernelPicker() {
    ...
    const language =
      metadata?.kernelspec?.language || metadata?.language_info?.name || null;
  ...
  runCellAndAdvance() {
    delegateToNotebook(this, "runCellAndAdvance");
  },
  runAllCells() {
