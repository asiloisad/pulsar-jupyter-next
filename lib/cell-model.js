...
class CellModel {
  ...
  setRunning(running) {
    ...
    if (running) {
      ...
      if (shared?.createExecutionTimeTracker) {
  ...
  addOutput(output) {
    ...
    // Truncate large stream outputs to prevent crashes
    if (normalizedOutput.output_type === "stream" && shared.truncateOutput) {
      const { text, truncated } = shared.truncateOutput(normalizedOutput.text);
      normalizedOutput.text = text;
      if (truncated) {
