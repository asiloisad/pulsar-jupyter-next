...
class CellModel {
  ...
  setRunning() {
    // Defer flipping status to "running" so cells that finish in <150ms never
  ...
  clearRunning() {
    ...
    // is still pending: flush it now so we don't leave stale previous-run
    // outputs lingering after the cell has visibly completed.
    if (this._pendingClearTimer) {
      clearTimeout(this._pendingClearTimer);
      this._pendingClearTimer = null;
      const opts = this._pendingClearOptions || {};
  ...
  // Defer the wipe so instant cells don't flash an empty output area between
  // "previous outputs" and "new outputs".  If new output arrives before the
  // timer fires, addOutput flushes the clear synchronously so the new output
  // replaces (not appends to) the previous run.  If the timer fires with no
  // new output, the clear happens normally.
  scheduleClearOutputs(options = {}, delayMs = 150) {
