...
class OutputView {
   ...
   * Uses hydrogen-next's shared utilities for ANSI, HTML, and MIME handling
   */
  renderFallback() {
    const shared = getHydrogenShared();
    if (!shared) {
      console.error("[jupyter-next] hydrogen-next is required but not available");
      return;
    }

    // Use shared utilities directly - hydrogen-next is required
    const { ansiToHtml, sanitizeHtml, truncateOutput, getBestMimeType, MIME_TYPE_PRIORITY } = shared;
    ...
    outputs.forEach((output) => {
      ...
      switch (output.output_type) {
        case "stream":
          ...
          let streamText = Array.isArray(output.text) ? output.text.join("") : output.text;
          // Truncate large outputs to prevent crashes
          const { text: truncatedText, truncated } = truncateOutput(streamText);
          pre.innerHTML = ansiToHtml(truncatedText);
          if (truncated) {
            const truncMsg = document.createElement("div");
            truncMsg.className = "output-truncated";
            truncMsg.textContent = "... [Output truncated]";
            div.appendChild(pre);
            div.appendChild(truncMsg);
          } else {
            div.appendChild(pre);
          }
          break;
        ...
        case "display_data":
          this.renderDisplayDataFallback(
            ...
            output.data,
            output.metadata,
            { sanitizeHtml, ansiToHtml, truncateOutput, getBestMimeType }
          );
          break;
  ...
  renderDisplayDataFallback(container, data, metadata, utils) {
    if (!data) return;

    const { sanitizeHtml, ansiToHtml, truncateOutput, getBestMimeType } = utils;

    // Use shared MIME type selection from hydrogen-next
    ...
    if (mimeType === "text/html") {
      ...
      // Apply width/height from metadata if available
      const mimeMetadata = metadata && metadata[mimeType];
      if (mimeMetadata) {
        if (mimeMetadata.width) {
          img.style.width = typeof mimeMetadata.width === "number"
            ? `${mimeMetadata.width}px`
            : mimeMetadata.width;
        }
        if (mimeMetadata.height) {
          img.style.height = typeof mimeMetadata.height === "number"
            ? `${mimeMetadata.height}px`
            : mimeMetadata.height;
        }
      }
      ...
      try {
        ...
        jsonPre.textContent = String(content);
      }
      container.appendChild(jsonPre);
    } else {
      // text/plain, text/latex, text/markdown, and others
      const textPre = document.createElement("pre");
      textPre.className = "output-text";
      // Truncate large text outputs
      const { text: truncatedContent, truncated } = truncateOutput(content);
      textPre.innerHTML = ansiToHtml(truncatedContent);
