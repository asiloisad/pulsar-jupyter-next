...
function appendMimeBundle(parent, output) {
  ...
  if (data["application/json"]) {
    appendText(parent, "output-json output-code", JSON.stringify(data["application/json"], null, 2));
    return;
  }
...
class OutputView {
  ...
  renderOutput(output, index) {
    ...
    if (output.output_type === "stream") {
      ...
      const traceback = output.traceback && output.traceback.length
        ? output.traceback
