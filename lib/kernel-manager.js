...
class KernelManager {
  ...
  async getKernelSpecs() {
    ...
    if (!specs || specs.length === 0) {
      throw new Error('No kernel specs found. Please install a Jupyter kernel (e.g., python -m pip install ipykernel)')
    }
  ...
  async getKernelSpecsForLanguage(language) {
    ...
    const languageMappings = atom.config.get('hydrogen-next.languageMappings') || {}
  ...
  async _startHydrogenKernel(spec, editor) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const filePath = editor?.getPath?.() || `jupyter-next-${spec.name}-${Date.now()}`
        ...
        hydrogenKM.startKernel(spec.spec, grammar, mockEditor, filePath, (hydrogenKernel) => {
          const kernel = new HydrogenKernelWrapper(hydrogenKernel, spec, this)

          this.kernels.set(spec.name, kernel)

          kernel.onDidChangeStatus((status) => {
            this.emitter.emit('did-change-status', { kernelName: spec.name, status })
          })
...
class HydrogenKernelWrapper {
  ...
  _subscribeToKernel() {
    ...
    if (kernel.onDidChangeExecutionState) {
      this._executionStateSubscription = kernel.onDidChangeExecutionState((state) => {
      ...
      this._executionStateSubscription = kernel.transport.onDidChangeExecutionState((state) => {
        if (this.hydrogenKernel) {
          this.setStatus(state)
        }
      })
      ...
      try {
        this.hydrogenKernel.execute(code, (result) => {
          ...
          try {
            ...
            console.error('[jupyter-next] Error processing execution result:', e)
          }
        })
      } catch (e) {
        cleanup()
