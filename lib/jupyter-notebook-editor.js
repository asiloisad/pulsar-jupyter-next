...
class JupyterNotebookEditor {
    ...
    this._loadingPromise = (async () => {
      try {
        ...
        if (state.filePath && state.notebookData && state.wasModified) {
          ...
          doc = new NotebookDocumentClass(state.filePath);
  ...
  commitSourceEditorSnapshot(reason = "sync", options = {}) {
    ...
    const text = JSON.stringify(
      this.getSourceEditorJSON({ includeHistoryState: true }),
      null,
      2,
    );
  ...
  applySourceEditorSnapshot(reason = "source-editor-change") {
    ...
    const isSourceHistory =
      reason === "source-editor-undo" || reason === "source-editor-redo";
    const shouldRestoreUIState =
      isSourceHistory || reason === "deserialize-source-editor";
    const historyChange = isSourceHistory
      ? this.classifyHistoryChange(parsed.notebook)
      : null;
