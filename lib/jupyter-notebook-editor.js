...
class JupyterNotebookEditor {
    ...
    for (const paneContainer of [
      ...
      for (const pane of paneContainer.getPanes()) {
        for (const item of pane.getItems()) {
          if (
            item instanceof JupyterNotebookEditor &&
            item.view &&
            !item._destroyed
          ) {
        ...
        try {
          ...
          if (buffer && buffer.getLineCount() > 0) {
            ...
            const validColumn = Math.min(
              position.column,
              buffer.lineLengthForRow(validRow) || 0
            );
            activeCellView.editor.setCursorBufferPosition([
              validRow,
              validColumn,
            ]);
          ...
          console.warn(
            "[jupyter-next] Could not restore cursor position:",
            e.message
          );
        }
      }
  ...
  async runAllCells() {
    if (!this.document) return;
    const interruptOnError = atom.config.get(
      "jupyter-next.execution.interruptOnError"
    );
  ...
  cutCell() {
    ...
    this.cellClipboard = indicesToCut.map((i) =>
      this.document.getCell(i).toJSON()
    );
  ...
  copyCell() {
    ...
    // Copy cells to clipboard
    this.cellClipboard = indicesToCopy.map((i) =>
      this.document.getCell(i).toJSON()
    );
  }
  ...
  duplicateCell() {
    ...
    const cellsData = indicesToDuplicate.map((i) =>
      this.document.getCell(i).toJSON()
    );
  ...
  async exportToHtml() {
    ...
    lines.push(
      ".cell { margin: 10px 0; border: 1px solid #ddd; border-radius: 4px; }"
    );
    lines.push(
    ...
    lines.push(
      ".cell-output { padding: 10px; border-top: 1px solid #ddd; background: #fff; }"
    );
    lines.push(
      ".execution-count { color: #888; font-size: 12px; padding: 5px 10px; }"
    );
    lines.push("</style>");
    ...
    for (let i = 0; i < this.document.getCellCount(); i++) {
      ...
      if (cell.type === "code") {
        if (cell.executionCount) {
          lines.push(
            `<div class="execution-count">In [${cell.executionCount}]:</div>`
          );
        }
        lines.push(
          `<div class="cell-code">${this.escapeHtml(cell.source)}</div>`
        );
