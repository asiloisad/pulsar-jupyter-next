...
class JupyterNotebookEditor {
  ...
  _initWithDocument(notebookDocument) {
    ...
    // Reconnect to a still-running kernel (e.g. file was closed and reopened)
    this._tryReconnectKernel();
  }

  _tryReconnectKernel() {
    const kernelName = this.document?.metadata?.kernelspec?.name;
    if (!kernelName) return;
    const existingKernel = this.document.kernelManager?.kernels?.get(kernelName);
    if (existingKernel?.isAlive()) {
      this.document.connectToKernel({ name: kernelName }).catch((e) => {
        console.error("[jupyter-next] Auto-reconnect to kernel failed:", e);
      });
    }
  }
