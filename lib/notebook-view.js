...
class NotebookView {
  constructor(props) {
    ...
    this._scrollPastEndDisposable = atom.config.onDidChange(
  ...
  buildToolbar() {
    ...
    cellTypeSelect.onchange = (e) =>
      editor && editor.changeCellType(e.target.value);
    ...
    kernelStatus.className = `kernel-status ${
      kernel ? kernel.status : "disconnected"
    }`;
    ...
    cellsArray.forEach((cell, index) => {
      ...
      const cellProps = {
        ...
        onSourceChange: (source) =>
          editor && editor.updateCellSource(index, source),
        ...
        onNavigateToPreviousCell: () => {
          if (editor && index > 0) {
            ...
            requestAnimationFrame(() => {
              const prevCellView = this.cellViews.get(
                cellsArray[index - 1]?.id
              );
              if (prevCellView) {
                prevCellView.focus();
                if (prevCellView.editor) {
                  const lastRow = prevCellView.editor.getLastBufferRow();
                  prevCellView.editor.setCursorBufferPosition([
                    lastRow,
                    Infinity,
                  ]);
                }
        ...
        onNavigateToNextCell: () => {
          if (editor && index < cellsArray.length - 1) {
            ...
            requestAnimationFrame(() => {
              const nextCellView = this.cellViews.get(
                cellsArray[index + 1]?.id
              );
              if (nextCellView) {
                nextCellView.focus();
