...
class NotebookView {
  ...
  buildToolbar() {
    ...
    // Run and move down button
    const runMoveDownBtn = document.createElement("button");
    runMoveDownBtn.className = "btn btn-sm icon icon-move-down";
    runMoveDownBtn.title = "Run Cell and Move Down (Shift+Enter)";
    runMoveDownBtn.onclick = () => editor && editor.runCellAndMoveDown();
    toolbarLeft.appendChild(runMoveDownBtn);
    ...
    cellTypeSelect.title = "Cell Type (scroll to cycle)";
  ...
  handleFocusIn(event) {
    ...
    // Skip mode change if programmatically focusing (e.g., run-and-move-down)
